function GenerateKPModelOutputFiles_OptimKT_060921(OptimParamCombos, StreamSgmnts, ObsKPDistances, ModelKPDistances, OutputFilenamePrefix)
...
%
%   ModelDistances - M x N matrix with modeled knickpoint propagation
%   distances. Each of the M rows represents an instance of StreamSgmnts
...
%   propagation. See Code in RunKPModel_OptimKT_060921 for details as to
...
for i = 1:size(OptimParamCombos,1)
    ...
    %%%Create matrix for modeled knickpoint locations (X, Y, upstream propagation distance, and StreamNumber)
    ModeledKPLocations = zeros(size(ObsKPDistances,1),4);
    ...
    for j = 1:size(ObsKPDistances,1)
        ...
        ModeledKPLocations(j,:) = [X_KP Y_KP L_KP ObsKPDistances(j,2)];     
    
    end  
    
    T = array2table(ModeledKPLocations);
    T.Properties.VariableNames(1:4) = {'X', 'Y', 'L_m_Upstream', 'StreamNumber'};
