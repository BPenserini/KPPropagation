function [KP_distance, OptimParamCombos, Misfit, sum_of_misfit] = RunKPModel_OptimKT_060821(Obs_Distances, StreamSgmnts, A, m, Kmin, Kmax, Tcap_min, Tcap_max)
...
%     for j = 1:length(Tcap)
    for j = 1:length(K)
        ...
        for n = 1:length(StreamSgmnts)
            ...
            KP_distance(n,((i-1)*length(K) + j)) = KPPropagation_051721(L_vec, A_vec, K(j), m, Tcap(i));
