function [OptimParamCombos] = RunKPModel_OptimKT_060821(Obs_Distances, StreamSgmnts, A, m, Kmin, Kmax, Tcap_min, Tcap_max)
...
%   Tcap_min - Minimum value of the age of capture in model
%   Tcap_max - Maximum value of the age of capture in model
...
K = logspace(log10(Kmin), log10(Kmax), 50); %Generate a log spaced array of K values to test, based on input
Tcap = linspace(Tcap_min, Tcap_max, 10); %Generate a linearly spaced array of capture age (Tcap) values to test, based on input

% Pre-allocate size of output matrix KP_distance
KP_distance = zeros(length(StreamSgmnts), length(K) * length(Tcap));
...
%     for j = 1:length(Tcap)
    for j = 1:length(K)
        ...
        for n = 1:length(StreamSgmnts)
            ...
            KP_distance(n,((i-1)*10 + j)) = KPPropagation_051721(L_vec, A_vec, K(j), m, Tcap(i));
...
Misfit = zeros(size(Obs_Distances,1),length(K) * length(Tcap)); %Initiates a matrix with height equal to the number of observed knickpoints and width equal to the number of modeled K,Tcap pairs to record the differences between modeled and observed KP upstream distance 
OptimParamCombos = cell(length(K),2); %Intiates a cell array to record the optimal combinations of K and Tcap. This enables there to be multiple best fit K values, in the case that optimal is an array with multiple equally good fits (Note: this may likely occur when there's a very poor fit).
...
for i = 1:length(Tcap)
    optimal = find(sum_of_misfit(((i-1)*length(K) + 1):(i*length(K))) == min(sum_of_misfit(((i-1)*length(K) + 1):(i*length(K))))); %Finds the index of the K with the lowest misfit for a given Tcap value. Subindexed array represents misfits for all Tcaps run for a given K.
    
    OptimParamCombos{i,2} = Tcap(i); %K(i);
    OptimParamCombos{i,1} = K(optimal); %Tcap(optimal);
end

%%%%%%%%%%%CONTINUE HERE WITH EXTRA CODE TO OUTPUT FILES
